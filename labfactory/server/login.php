...
session_start();
	...
	mysql_connect($server,$user,$passer) or die("Ошибка подключения");
	mysql_select_db($db) or die("Отсутствует база данных");
	$login1 = mysql_real_escape_string($login);
	$pass = mysql_real_escape_string($pass);
	//спомощью запроса получаем все данные из таблицы users где логины совпадают.
	$query = mysql_query("SELECT * FROM users WHERE login='$login1'");
