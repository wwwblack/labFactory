...
switch ($userlist){
	...
	";				
							...
							while ($result = mysql_fetch_array($result_set)) {
								echo "<tr><form method=\"POST\">";
									...
									echo "<td>".$result["surname_of_user"]." ".$result["name_of_user"]." ".$result["patronymic_of_user"]."</td>";
									echo "<td>".$result["e-mail"]."</td>";
									echo "<td>".$result["mobile_phone"]."</td>";
									$resultidrole = $result["id_role_of_user"];
									switch ($resultidrole){
									case 1:
	...
	case 2:
	//тут мы вызываем информацию об оборудовании, можно редактировать, и удалить не нужное оборудование	
	echo "
		...
		<div class=\"row\" >
			...
			<div style = \"text-align: center;\">
							...
							<tr>
								...
								<td border=\"2\">
								<button class=\"btn btn-primary btn-sm\" id=\"add_room\" value\"3\">Добавить</button>
								</td>
							</tr>
							<tr>
								<td>
									<textarea rows=\"5\" cols=\"40\" wrap=\"soft\" placeholder=\"Введите описание\">
							...
							<table border=\"1\" style=\"margin: 0 auto;\" width=\"50%\">
							<tr>
								<td border=\"2\">Название оборудования
								</td>
	...
	";				
							$result_set = mysql_query("SELECT * FROM device");
							while ($result = mysql_fetch_array($result_set)) {
								echo "
								<script type=\"text/javascript\">
									...
									<tr>
									<td>".$result["name_device"]."</td>
									<td><input id=\"update".$result["id"]."\" type=\"text\" name=\"position_name\" size=\"30%\" autocomplete=\"off\"  placeholder=\"".$result["name_device"]."\"></td>
									<td><button  class=\"btn btn-primary btn-sm\" id=\"button_update_position_name".$result["id"]."\" value= \"".$result["id"]."\">Редактировать </button></td>
									<td><button class=\"btn btn-primary btn-sm\" id=\"delete_posiotion_name".$result["id"]."\" value=".$result["id"].">Удалить</button></td>
								 </tr>";
							}		
	echo "						
							</table>
